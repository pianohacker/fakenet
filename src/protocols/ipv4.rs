...
proto_enum_with_unknown!(IpProtocolNumber, u8, {
    Udp = 17,
    Ipv6Icmp = 58,
});
...
pub fn ipv4_address<'a>(input: &'a [u8]) -> BIResult<'a, Ipv4Address> {
...
mod tests {
    ...
    fn nonzero_address_decodes() {
        assert_eq!(
            "1.2.3.4".parse::<Ipv4Address>().unwrap(),
            Ipv4Address([1, 2, 3, 4])
        );
    }
    ...
    fn address_with_zeroes_decodes() {
        assert_eq!(
            "10.0.3.0".parse::<Ipv4Address>().unwrap(),
            Ipv4Address([10, 0, 3, 0])
        );
    }
}
